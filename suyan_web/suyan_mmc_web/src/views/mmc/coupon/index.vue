<template>
  <div style="padding-right: 20px;">
    <Form :model="dataForm" inline>
      <FormItem>
        <Input type="text" v-model="dataForm.id" placeholder="优惠券id">
        </Input>
      </FormItem>
      <FormItem>
        <Input type="text" v-model="dataForm.couponName" placeholder="优惠券名称">
        </Input>
      </FormItem>
      <FormItem>
        <Button @click="getDataList()">查询</Button>
        <Button type="primary" @click="$router.push({name: 'mmcCouponCreate'});" style="margin-left: 10px;">创建优惠券</Button>
      </FormItem>
    </Form>
  ...
  export default {
    data () {
      return {
        ...
        columns: [
          ...
          {
            title: '名称', key: 'couponName'
          },
          {
            title: '编码', key: 'couponNo'
          },
          {
          ...
          {
            title: '状态', key: 'couponStatusDesc', width: 120
          },
          {
            title: '操作',
            key: 'action',
            ...
            render: (h, params) => {
              ...
              var actionButtons = [h('Button', {
                ...
                on: {
                  click: () => {
                    this.$router.push({name: 'mmcCouponShow', params: {id: params.row.id}});
                  }
                }
              }, '查看')];
              if (params.row.couponStatus == 1) {
                actionButtons.push(h('Button', {
                  ...
                  on: {
                    click: () => {
                      this.$router.push({name: 'mmcCouponCreate', params: {id: params.row.id}});
                    }
                  }
                }, '编辑'));
                actionButtons.push(h('Button', {
                  ...
                  on: {
                    click: () => {
                      this.$Modal.confirm({
                        ...
                        onOk: () => {
                          this.$http({
                            url: this.$http.adornUrl('coupon/changeStatus/'+params.row.id+'/1'),
                            method: 'post'
                          }).then(({data}) => {
                  ...
                  on: {
                    click: () => {
                      this.$Modal.confirm({
                        ...
                        onOk: () => {
                          this.$http({
                            url: this.$http.adornUrl('coupon/changeStatus/'+params.row.id+'/2'),
                            method: 'post'
                          }).then(({data}) => {
                            if (data && data.code === 200) {
                              this.$Message.info('操作成功');
              ...
              if (params.row.couponStatus == 2 || params.row.couponStatus == 3) {
                actionButtons.push(h('Button', {
                  style: {
                    marginLeft: '10px'
                  },
                  ...
                  on: {
                    click: () => {
                      this.$Modal.confirm({
                        ...
                        onOk: () => {
                          this.$http({
                            url: this.$http.adornUrl('coupon/changeStatus/'+params.row.id+'/3'),
                            method: 'post'
                          }).then(({data}) => {
                            if (data && data.code === 200) {
                              this.$Message.info('操作成功');
              ...
              if (params.row.couponStatus == 2 || params.row.couponStatus == 3 || params.row.couponStatus == 4) {
                actionButtons.push(h('Button', {
                  style: {
                    marginLeft: '10px'
                  },
                  ...
                  on: {
                    click: () => {
                      this.$Modal.confirm({
                        ...
                        onOk: () => {
                          this.$http({
                            url: this.$http.adornUrl('coupon/changeStatus/'+params.row.id+'/4'),
                            method: 'post'
                          }).then(({data}) => {
                            if (data && data.code === 200) {
                              this.$Message.info('操作成功');
    ...
    methods: {
      ...
      getDataList () {
        this.loading = true;
        this.$http({
          url: this.$http.adornUrl('coupon/queryCoupon'),
          method: 'post',
          data: this.$http.adornData({
            ...
            'order': 'desc',
            'id': this.dataForm.id,
            'couponNameLike': this.dataForm.couponName,
          })
        }).then(({data}) => {
